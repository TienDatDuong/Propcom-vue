<template>
  <div class="group-detail-my-post-component">
    <TabGroup
      ...
      <TabList as="template">
        <div class="app-tab">
          <Tab v-slot="{ selected }" as="template">
            <div class="app-tab-btn" :class="selected && 'active'">Bài đang chờ</div>
          </Tab>
          <Tab v-slot="{ selected }" as="template">
            <div class="app-tab-btn" :class="selected && 'active'">Bài đã đăng</div>
          </Tab>
          <Tab v-slot="{ selected }" as="template">
            <div class="app-tab-btn" :class="selected && 'active'">Bài đã bị ẩn</div>
          </Tab>
        </div>
      </TabList>
      <TabPanels>
        <TabPanel>
          ...
          <template v-else>
            ...
            <template v-if="data.waiting.totalItems && data.waiting.totalItems.length">
              <div class="group-detail-my-post-component__container" ref="scrollWaitingComponent">
        ...
        <TabPanel>
          ...
          <template v-else>
            ...
            <template v-if="data.active.totalItems && data.active.totalItems.length">
              <div class="group-detail-my-post-component__container" ref="scrollActiveComponent">
        ...
        <TabPanel>
          ...
          <template v-else>
            ...
            <template v-if="data.inactive.totalItems && data.inactive.totalItems.length">
              <div class="group-detail-my-post-component__container" ref="scrollInActiveComponent">
...
.group-detail-my-post-component {
  width: 100%;
  &__tabgroup {
    display: flex;
...
import { GroupModel, GroupPostModel, GroupPostStatusEnum, GroupPostTypeEnum } from '@/services/features/group/group.model';
...
class GroupDetailMyPostComponentProps {
  dataSource?: GroupModel;
}

// Định nghĩa data type của component [data] bằng Typescript
class GroupDetailMyPostComponentData {
  isLoading: boolean;
  tabDefault: number;
  waiting: {
    isLoading: boolean;
...
export default defineComponent({
  name: 'GroupDetailMyPostComponent',
  components: {
    TabGroup,
    TabList,
    Tab,
  ...
  setup(props: GroupDetailMyPostComponentProps, { emit }: any) {
    const { base } = useInit();
    const data = reactive<GroupDetailMyPostComponentData>({
      isLoading: false,
      tabDefault: 0,
      waiting: {
        isLoading: false,
        ...
        paging: null,
        totalItems: null,
        type: GroupPostTypeEnum.MY_POST_WAITING,
      },
      active: {
        isLoading: false,
        isLoadingMore: false,
        ...
        paging: null,
        totalItems: null,
        type: GroupPostTypeEnum.MY_POST_ACTIVE,
      },
      inactive: {
        isLoading: false,
        isLoadingMore: false,
        ...
        paging: null,
        totalItems: null,
        type: GroupPostTypeEnum.MY_POST_INACTIVE,
      },
    });
    // Template ref
    ...
    const loadWaitingPost = async (id: string) => {
      ...
      try {
        ...
        data.waiting.queryParams.isMyPost = true;
        // Lấy chi tiết nhóm
    ...
    const loadInActivePost = async (id: string) => {
      ...
      try {
        ...
        data.inactive.queryParams.isMyPost = true;
        // Lấy chi tiết nhóm
    ...
    const loadActivePost = async (id: string) => {
      ...
      try {
        ...
        data.active.queryParams.isMyPost = true;
        // Lấy chi tiết nhóm
