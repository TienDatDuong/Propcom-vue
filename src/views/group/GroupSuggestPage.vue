...
export default defineComponent({
  ...
  setup() {
    ...
    const data = reactive<GroupSuggestPageData>({
      isLoading: false,
      routeQueryParams: {
        page: '1',
        pageSize: '10',
    ...
    const loadData = async (queryParams: any) => {
      ...
      try {
        if (isLoggedIn.value) {
          data.paging = await findGroupSuggestByCurrentEmployee(queryParams);
        } else {
          data.paging = await findGroupSuggestByCurrentEmployeeNotLogin(queryParams);
        }
